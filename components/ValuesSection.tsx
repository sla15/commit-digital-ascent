      ...
      <div className="relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-24 gap-10">
          <div className="max-w-2xl">
            <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="text-brand-400 font-bold uppercase tracking-[0.2em] text-sm mb-4">Our Foundation</motion.h2>
            <motion.h3 initial={{ opacity: 0, y: getEntranceOffset(80, 20) }} whileInView={{ opacity: 1, y: 0 }} className="text-4xl md:text-6xl font-display font-bold mb-8 leading-tight">Built on Professional <br/> Integrity</motion.h3>
            <motion.p initial={{ opacity: 0, y: getEntranceOffset(80, 20) }} whileInView={{ opacity: 1, y: 0 }} className="text-brand-100 text-xl leading-relaxed">Our values drive every solution we build and every partnership we cultivate.</motion.p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-8">
          {displayValues.map((value, index) => (
            ...
            >
              ...
              <p className="text-brand-200 text-lg leading-relaxed opacity-80 group-hover:opacity-100 transition-opacity">{value.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
