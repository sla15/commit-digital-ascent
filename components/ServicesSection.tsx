...
export const ServicesSection: React.FC = () => {
  ...
  // Merge dynamic content with static icon data
            ...
            <div className="relative z-10 flex flex-col">
              ...
              <Link to="/services">
                <Button variant="secondary" className="w-full justify-between py-4 bg-slate-50 border-slate-100 transition-all rounded-[1.5rem]">
                  ...
                  <div className="bg-brand-900 text-white p-2.5 rounded-full"><ArrowRight className="w-4 h-4" /></div>
                </Button>
              </Link>
            </div>
          </div>
      ...
      <div className="hidden md:grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-14">
        {displayServices.map((service, index) => (
          <motion.div
            ...
            viewport={{ once: true, amount: 0.4 }}
            transition={{ duration: 0.7, delay: index * 0.15, type: "spring", stiffness: 60, damping: 20 }}
          ...
          >
            ...
            <div className="relative z-10 flex flex-col h-full">
              ...
              <h4 className="text-3xl font-bold text-slate-900 mb-5 group-hover:text-brand-700 transition-colors font-display tracking-tight">{service.title}</h4>
              <p className="text-slate-600 leading-relaxed mb-12 flex-grow text-lg font-medium opacity-80 group-hover:opacity-100 transition-opacity">{service.description}</p>
              <Link to="/services">
                <Button variant="secondary" className="w-full justify-between py-6 bg-slate-50 border-slate-100 group-hover:bg-brand-50 group-hover:border-brand-200 transition-all rounded-[1.5rem] group-hover:shadow-lg">
                  <span className="font-bold">Learn More</span>
                  <div className="bg-brand-900 text-white p-2.5 rounded-full group-hover:translate-x-1 transition-transform"><ArrowRight className="w-5 h-5" /></div>
                </Button>
              </Link>
            </div>
          </motion.div>
